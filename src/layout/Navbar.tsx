  ...
  return (
    <header className="fixed top-4 left-0 w-full z-50 px-4 flex justify-center pointer-events-none">
      <motion.div
        ...
        className="pointer-events-auto relative rounded-[32px] p-[1px] group/nav overflow-visible transition-shadow duration-300 hover:shadow-[0_20px_60px_rgba(255,255,255,0.12)] shadow-[0_20px_50px_rgba(0,0,0,0.6)]"
      >
        {/* Navbar Gradient Border */}
        <div className="absolute inset-0 rounded-[32px] bg-gradient-to-r from-white via-neutral-300 to-neutral-600 opacity-40 group-hover/nav:opacity-80 transition-opacity duration-500 shadow-[0_0_15px_rgba(255,255,255,0.15)]" />
          ...
          <div className="flex items-center justify-between px-4 py-2 sm:px-6 sm:py-2.5 gap-4 sm:gap-6 min-h-[48px]">
            ...
            <a href="#" className="flex items-center gap-2.5 group shrink-0">
              <div className="relative flex items-center justify-center">
                <div className="w-8 h-8 rounded-full bg-white text-black flex items-center justify-center font-black text-xs shadow-[0_0_12px_rgba(255,255,255,0.4)] group-hover:scale-105 transition-transform">
                  N
                </div>
                <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-white border-2 border-[#070709] rounded-full animate-pulse shadow-[0_0_8px_rgba(255,255,255,0.8)]" />
              </div>
              ...
              <div className="flex flex-col">
                <span className="font-black text-sm sm:text-base uppercase tracking-tight text-white group-hover:text-[#D7E2EA] transition-colors leading-none">
                  Nexora<span className="text-white/60">Now</span>
                </span>
                <span className="hidden sm:flex items-center gap-1 text-[9px] font-medium tracking-wider uppercase text-white/70 leading-tight">
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              {!isOpen && (
                <span className="hidden md:inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-[10px] uppercase font-semibold text-gray-300 border border-white/10">
                  <Sparkles size={10} className="text-white" /> AI Agency
                </span>
              )}
              ...
              >
                <div className="relative h-full w-full bg-[#0A0A0D]/90 backdrop-blur-md px-4 py-2 sm:px-5 sm:py-2 flex items-center gap-1.5 transition-colors duration-500 border border-white/15 hover:border-white/60 rounded-full shadow-md">
                  <div className="absolute inset-0 bg-gradient-to-r from-white/20 via-white/10 to-white/5 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-500 pointer-events-none" />
              ...
              >
                {navLinks.map((link) => (
                  <a
                    ...
                    className="text-sm font-semibold uppercase tracking-wider text-gray-300 hover:text-white transition-colors py-1.5 border-b border-white/5"
                  >
                    {link.label}
                  </a>
                ))}
                <div className="pt-2 flex justify-between items-center text-xs text-gray-400">
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-white animate-ping" /> Live Status: Online
                  </span>
                  <span className="text-white font-semibold">NexoraNow</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
