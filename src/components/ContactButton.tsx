...
export function ContactButton({ label = "Book a Free Consultation", onClick, className = "" }: ContactButtonProps) {
  ...
  return (
    ...
    >
      {/* Top Left Bracket */}
      <div className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-white group-hover:w-full group-hover:h-full group-hover:border-white/50 transition-all duration-500 z-20 pointer-events-none" />
      
      {/* Bottom Right Bracket */}
      <div className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-white/70 group-hover:w-full group-hover:h-full group-hover:border-white/50 transition-all duration-500 z-20 pointer-events-none" />
